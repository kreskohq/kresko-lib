...
import {SafeMath} from "oz-contracts/utils/math/SafeMath.sol";
...
contract KrStaking is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant OPERATOR_ROLE =
        0x8952ae23cc3fea91b9dba0cefa16d18a26ca2bf124b54f42b5d04bce3aacecd2;
    ...
    function initialize(
        ...
        require(
            _rewardPerBlocks.length == _rewardTokens.length,
            "Reward tokens must have a rewardPerBlock value"
        );
    ...
    modifier ensurePoolDoesNotExist(IERC20 _depositToken) {
        for (uint256 i; i < _poolInfo.length; i++) {
            require(
                address(_poolInfo[i].depositToken) != address(_depositToken),
                "KR: poolExists"
            );
        }
        _;
    ...
    modifier ensurePoolExists(uint256 _pid) {
        require(
            address(_poolInfo[_pid].depositToken) != address(0),
            "KR: !poolExists"
        );
        _;
    }
    ...
    function pendingRewards(
        uint256 _pid,
        address _user
    ) public view returns (Reward memory rewards) {
        ...
        rewards = Reward({
            ...
            amounts: new uint256[](rewardTokensLength)
        });

        if (depositTokenSupply != 0 && user.amount > 0) {
            for (
                uint256 rewardIndex;
                rewardIndex < rewardTokensLength;
                rewardIndex++
            ) {
                uint256 accRewardPerShare = pool.accRewardPerShares[
                    rewardIndex
                ];

                uint256 rewardPerBlock = rewardPerBlockFor[
                    pool.rewardTokens[rewardIndex]
                ];
                uint256 blocks = block.number - pool.lastRewardBlock;
                uint256 reward = (rewardPerBlock * blocks * pool.allocPoint) /
                    totalAllocPoint;

                accRewardPerShare += (reward * 1e12) / depositTokenSupply;

                rewards.amounts[rewardIndex] =
                    (user.amount * accRewardPerShare) /
                    1e12 -
                    user.rewardDebts[rewardIndex];
    ...
    function allPendingRewards(
        address _account
    ) external view returns (Reward[] memory allRewards) {
    ...
    ) external view returns (uint256 pid, bool found) {
    ...
    function updatePool(
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (deposits > 0) {
                for (
                    uint256 rewardIndex;
                    rewardIndex < pool.rewardTokens.length;
                    rewardIndex++
                ) {
                    ...
                    pool.accRewardPerShares[rewardIndex] +=
                        (reward * 1e12) /
                        deposits;
                }
            }
    ...
    function deposit(
        ...
        for (
            ...
            user.rewardDebts[rewardIndex] +=
                (_amount * pool.accRewardPerShares[rewardIndex]) /
                1e12;
        }
    ...
    function withdraw(
        ...
        for (
            ...
            user.rewardDebts[rewardIndex] =
                (user.amount * pool.accRewardPerShares[rewardIndex]) /
                1e12;
        }
    ...
    function addPool(
        ...
        ensurePoolDoesNotExist(_depositToken)
    {
    ...
    function sendRewards(
        ...
        for (
            ...
            uint256 rewardDebt = (user.amount *
                pool.accRewardPerShares[rewardIndex]) / 1e12;
            uint256 pending = rewardDebt - user.rewardDebts[rewardIndex];

            if (pending > 0) {
                IERC20(pool.rewardTokens[rewardIndex]).safeTransfer(
                    recipient,
                    pending
                );
                emit ClaimRewards(
