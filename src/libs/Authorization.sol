...
import {ds} from "../types/DiamondTypes.sol";
...
library Role {
    ...
    bytes32 constant SAFETY_COUNCIL =
...
library Authorization {
    ...
    function setupSecurityCouncil(address _councilAddress) internal {
        require(
            getRoleMemberCount(Role.SAFETY_COUNCIL) == 0,
            Error.SAFETY_COUNCIL_EXISTS
        );
        require(
            IGnosisSafeL2(_councilAddress).isOwner(msg.sender),
            Error.ADDRESS_INVALID_SAFETY_COUNCIL
        );

        ds()._roles[Role.SAFETY_COUNCIL].members[_councilAddress] = true;
        ds()._roleMembers[Role.SAFETY_COUNCIL].add(_councilAddress);

        emit AuthEvent.RoleGranted(
            Role.SAFETY_COUNCIL,
            _councilAddress,
            Meta.msgSender()
        );
