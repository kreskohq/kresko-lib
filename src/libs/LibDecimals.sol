...
library LibDecimals {
    ...
    function toCollateralFixedPointAmount(
        ...
        FixedPoint.Unsigned memory fixedPointAmount = FixedPoint.Unsigned(
            _amount
        );
        ...
        if (_collateralAssetDecimals < FixedPoint.FP_DECIMALS) {
            ...
            return
                fixedPointAmount.mul(
                    10 ** (FixedPoint.FP_DECIMALS - _collateralAssetDecimals)
                );
            ...
            return
                fixedPointAmount.div(
                    10 ** (_collateralAssetDecimals - FixedPoint.FP_DECIMALS)
                );
        }
