...
contract FluxPriceFeedFactory {
    ...
    function transmit(
        ...
        for (uint256 i = 0; i < _pricePairs.length; i++) {
            ...
            require(
                fluxPriceFeeds[id].hasRole(VALIDATOR_ROLE, msg.sender),
                "Only validators can transmit"
            );

            // try transmitting values to the oracle
            /* solhint-disable-next-line no-empty-blocks */
            try
                fluxPriceFeeds[id].transmit(
                    _answers[i],
                    _marketStatusAnswers[i]
                )
            {
    ...
    function _deployOracle(
        ...
        FluxPriceFeed newPriceFeed = new FluxPriceFeed(
            address(this),
            _decimals,
            _pricePair
        );
    ...
    function valueFor(
        bytes32 _id
    ) external view returns (int256, bool, uint256, uint256) {
